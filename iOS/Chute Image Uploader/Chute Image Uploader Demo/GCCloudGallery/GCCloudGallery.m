...
@implementation GCCloudGallery
@synthesize objects, sliderObjects, currentPage, objectSlider;

- (CGRect)rectForPage:(NSInteger)page{
    CGRect imageRect = CGRectMake((objectSlider.frame.size.width*page), 0, objectSlider.frame.size.width, objectSlider.frame.size.height);
    return imageRect;
}
...
- (void) queueObjectRetrievalForPage:(NSInteger)page
{
    ...
    [v setFrame:[self rectForPage:(page-1)]];
    v.tag = page;
...
-(UIView*) viewForPage:(NSInteger)page{
    ...
    [asset imageForWidth:320 andHeight:480 inBackgroundWithCompletion:^(UIImage *temp){
        [image setImage:temp];
    }];
...
- (void)switchToObjectAtIndex:(NSUInteger)index animated:(BOOL)_animated{
    ...
    [objectSlider scrollRectToVisible:[self rectForPage:(index-1)] animated:_animated];
